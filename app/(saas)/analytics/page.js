...
function StatusDonut({ byStatus, size = 132, thickness = 20 }) {
  ...
  // it. An earlier version tried to nudge both a segment's start AND end
  // (via stroke-dasharray + stroke-dashoffset on a stack of full circles)
  // to paper over the seam — that left a visible cut at some boundaries
  // depending on the split (e.g. a small slice next to a large one).
  // Extending only forward, never pulling a start back, removes that
  // ambiguity: there's exactly one seam-prone edge per boundary, and this
  // always covers it, regardless of how uneven the segments are.
...
// Super Admin only — gated the same way as /admin, via the superAdmin flag
// from /api/me. Also fetches isAccountAdmin from the same call (Team's
// visibility in AccountTabs depends on it) so navigating here doesn't make
// the Team tab disappear the way Admin's page once did.
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const r2 = await fetch("/api/admin/analytics", { headers: { Authorization: "Bearer " + t } });
        const d2 = await r2.json();
        if (!r2.ok) throw new Error(d2.error || "Could not load analytics.");
        setData(d2);
      } catch (e) {
        setIsSuper(false);
  ...
  if (!user || isSuper === null) {
    return (
      <div>
        <Nav />
        <AccountTabs active="analytics" isAccountAdmin={isAccountAdmin} isSuper={false} />
  ...
  return (
    <div>
      <Nav />
      <AccountTabs active="analytics" isAccountAdmin={isAccountAdmin} isSuper={true} />

      <div className="admin-shell">
        <h1 className="dash-heading" style={{ fontSize: 20 }}>Analytics</h1>
        <p className="dash-sub" style={{ marginBottom: 16 }}>Subscriptions and revenue across every organization.</p>
          ...
          <>
            ...
            <div style={{ display: "grid", gap: 16, gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))" }}>
              <div className="card">
                ...
                {/* auto-fit above stacks this card full-width once the viewport can't fit
                    two 300px+ cards side by side, so the donut + legend row never gets
                    squeezed to the point the legend clips or wraps out of view; justifyContent
                    keeps it centered whichever way it lays out. Donut sized up and the gap
                    widened so the legend's dots sit clearly clear of the ring instead of
                    crowding its right edge. */}
