...
import { appAccessDenial } from "./trial";
...
export async function resolveAccount(uid) {
  ...
  const hasOwner = ownerSnap.exists;
  const hasMembership = memSnap.exists;
  if (!hasOwner && !hasMembership) throw { status: 404, message: "No Bizzux account found for this sign-in" };

  // A login can both own its own Bizzux account AND be invited as staff on
  // someone else's team (e.g. an independent customer who's also a
  // teammate elsewhere). Default to their own account; honor an explicit
  // switch (users/{uid}.useMembershipContext, set via /api/switch-context)
  // so the membership side stays reachable too — see the account switcher
  // in Nav.
  let useMembership = false;
  if (hasOwner && hasMembership) {
    const userSnap = await adminDb().doc("users/" + uid).get();
    useMembership = userSnap.exists && userSnap.data().useMembershipContext === true;
  }

  if (hasOwner && !useMembership) {
    return {
      accountId: uid, profile: "Admin", isOwner: true, customer: ownerSnap.data(),
      organizationId: uid, organizationRole: "ORGANIZATION_OWNER", hasMembership,
    };
  }
...
// Checks only the env-var isSuper on the way in (not resolvePlatformRole) —
// this IS a real customer hot path (Notes/Files/Projects hit it on every
// load), so it intentionally doesn't add a Firestore read for the common
// case (an in-trial or paying customer). A Platform Admin created later via
// the platform portal (not in SUPER_ADMIN_EMAIL) falls through to the
// regular per-account checks below instead — but those now fall back to
// resolvePlatformRole before actually rejecting, so a Platform Admin still
// always gets in regardless of their own account's trial/plan status, just
// at the cost of one extra Firestore read only when that check would
// otherwise fail.
// Pass `appKey` to also check the account's plan covers that specific app.
export async function requireAccountWithAppsAccess(req, appKey) {
  const c = await requireUser(req);
  if (c.isSuper) {
    try {
      const acct = await resolveAccount(c.uid);
  ...
  let acct;
  try {
    acct = await resolveAccount(c.uid);
  } catch (e) {
    // No customers/ or memberships/ doc of their own — could still be a
    // portal-created Platform Admin (SUPER_ADMIN_EMAIL missed them above).
    const platformRole = await resolvePlatformRole(c.uid, c.email);
    if (platformRole === "OWNER" || platformRole === "ADMIN") {
      return { ...c, isSuper: true, accountId: c.uid, profile: "Admin", isOwner: true };
    }
    throw e;
  }
  ...
  const denial = await appDenialFor(customer, appKey);
  if (denial) {
    // Same portal-created Platform Admin case, but this time they DO have
    // their own (expired/suspended) customer record — check platformAdmins
    // before rejecting, so being a Platform Admin always beats an account's
    // own trial/plan status.
    const platformRole = await resolvePlatformRole(c.uid, c.email);
    if (platformRole !== "OWNER" && platformRole !== "ADMIN") throw denial;
    return { ...c, ...acct, isSuper: true };
  }
  return { ...c, ...acct };
}

// Trial/plan gate for one app. Only reads pricing (for the Suite's current
// app list) when the account actually has a Suite subscription, so the
// common trial / single-app path costs no extra read. Imported lazily:
// lib/pricing.js itself imports this file.
export async function appDenialFor(customer, appKey) {
  let suiteAppKeys;
  if (appKey && customer?.status === "active" && customer?.subscription?.planType === "SUITE") {
    try {
      const { getPricingConfig } = await import("./pricing");
      const { suiteApps } = await import("./pricingMath");
      suiteAppKeys = suiteApps((await getPricingConfig()).apps).map((a) => a.appKey);
    } catch {
      suiteAppKeys = undefined; // pricing unreadable: don't lock a paying Suite customer out
    }
  }
  return appAccessDenial(customer, appKey, suiteAppKeys);
}
