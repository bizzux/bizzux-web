...
export function buildSubscriptionSnapshot(quote, { discountedUnitPrice, couponCode, gateway, subscriptionId }) {
  ...
  // Annual plans are always locked for their 12-month term (the promise on
  // the pricing page, lib/pricingPolicy.js); a plan-level price lock can
  // extend it. Monthly plans have no fixed end: any change needs 30 days'
  // notice.
  const lockMonths = Math.max(Number(quote.priceLockMonths) || 0, quote.billingCycle === "year" ? 12 : 0) || null;
  let priceLockedUntil = null;
  if (lockMonths) {
    const d = new Date(agreedAt);
    d.setMonth(d.getMonth() + lockMonths);
    priceLockedUntil = d.toISOString();
  }
  const regular = Number(quote.regularUnitPrice) || Number(quote.unitPrice);
  return {
    regularUnitPrice: regular,
    discountPercent: regular > 0 ? Math.round(((regular - unitPrice) / regular) * 100) : 0,
    promotionApplied: !!quote.promotionApplied,
    promotionLabel: quote.promotionLabel || null,
    priceVersion: quote.priceVersion || 1,
    priceLockMonths: lockMonths,
    priceLockedUntil,
